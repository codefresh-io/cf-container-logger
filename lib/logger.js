...
class Logger {
    ...
    constructor(loggerId, firebaseAuthUrl, firebaseSecret, findExistingContainers, valuesToMask) {
        this.state                  = { status: 'init' };
        this.firebaseAuthUrl        = firebaseAuthUrl;
        this.firebaseSecret         = firebaseSecret;
        this.loggerId               = loggerId;
        this.findExistingContainers = findExistingContainers === 'true';
        if (valuesToMask.length) {
            this.valuesToMaskRegex      = new RegExp(`${valuesToMask.join('|')}`);
        }
    ...
    _handleContainer(container) {
        ...
        const containerLogger    = new ContainerLogger(containerId, containerInterface, firebaseLogger, firebaseLastUpdate, loggerStrategy, this.valuesToMaskRegex);
