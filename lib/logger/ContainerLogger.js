...
class ContainerLogger {
    ...
    constructor(containerId, containerInterface, receivedLogsLogger, loggerLastUpdate, loggerStrategy) {
        this.containerId = containerId;
        this.containerInterface = containerInterface;
        this.receivedLogsLogger = receivedLogsLogger;
        this.loggerLastUpdate = loggerLastUpdate;
    ...
    _handleTtyStream(stream, isError) {
        stream.on('data', (chunk) => {
            ...
            this._logMessageToLogger(message, isError);
        });
    ...
    _handleNonTtyStream(stream, isError) {
        stream.on('readable', () => {
            ...
            while (header !== null) {
                ...
                this._logMessageToLogger(new Buffer(payload).toString('utf8'), isError);
    ...
    _logMessageToLogger(message, isError) {
        if (isError) {
            message = `\x1B[31m${message}\x1B[0m`;
        }

        this.receivedLogsLogger.push(message);
        this.loggerLastUpdate.set(new Date().getTime());
    }
