...
class Logger {
    ...
    constructor({
        ...
        if (showProgress) {
            this.showProgressInterval = setInterval(this._showProgress.bind(this), 1000);
        }
    }
    ...
    _showProgress() {
        if (this.state.status === 'done' && this.showProgressInterval) {
            clearInterval(this.showProgressInterval);
        } else if (this.state.logsStatus) {
            logger.info(`logger progress update: ${this.state.logsStatus}`);
        }
    }
