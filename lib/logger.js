...
class Logger {
    ...
    constructor({
        ...
        this.buildFinishedPromise = buildFinishedPromise;
    ...
    async _handleContainer(container) { // jshint ignore:line
        ...
        containerLogger.on('end', _handleContainerStreamEnd.bind(this));
    ...
    _handleContainerStreamEnd() {
        ...
        if (this.containerLoggers.length === this.finishedContainers) {
            this.allStreamsClosed.promise.resolve();
        }
    }

    _handleBuildFinished() {
        this.buildFinishedPromise
            .then(() => this.allStreamsClosed.promise)
            .then(() => this.taskLogger.awaitLogsFlushed())
            .then(() => {
