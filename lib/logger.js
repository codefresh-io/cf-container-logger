...
class Logger {
    ...
    constructor({
        ...
        this.messageFilters         = this._buildMessageFilters();
    ...
    _buildMessageFilters() {
        const regex = {}; // store all regex to avoid repetition of creation them
        return _.map(_.get(this.taskLoggerConfig, 'opts.blackList', []), (variableToFilter) => {
            regex[variableToFilter] = new RegExp(variableToFilter, 'g');
            return (message) => {
                return message.replace(regex[variableToFilter], '****');
            };
        });
    }
