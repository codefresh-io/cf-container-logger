...
class Logger {
    ...
    _handleContainer(container) {
        ...
        const receivedFirebaseUrl = _.get(container, 'Labels', _.get(container, 'Actor.Attributes'))['io.codefresh.loggerFirebaseUrl'];
        const loggerStrategy      = _.get(container, 'Labels', _.get(container, 'Actor.Attributes'))['io.codefresh.loggerStrategy'];
        ...
        if (!loggerStrategy) {
            logger.warn(`Container: ${containerId} does contain a loggerStrategy label. skipping`);
            return;
        }

        if (LoggerStrategy.ALL.indexOf(loggerStrategy) === -1) {
            logger.warn(`Container: ${containerId}, loggerStrategy: '${loggerStrategy}' is not supported. skipping`);
            return;
        }
        ...
        let firebaseLogger;
        try {
            firebaseLogger = new Firebase(receivedFirebaseUrl);
        } catch (err) {
            const error = new CFError({
                cause: err,
