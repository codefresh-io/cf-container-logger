...
class ContainerLogger extends EventEmitter {
    ...
    _streamTty(stdout, stderr) {
        ...
        const logSizeLimitStream = this._logSizeLimitStream();
        const stepNameHeadersStream = this.stepLogger.stepNameTransformStream;
        const stepLoggerWritableStream = this.stepLogger.writeStream();

        logSizeLimitStream.on('error', err => logger.error(`logSizeLimitStream: ${err}`));
        stepNameHeadersStream.on('error', err => logger.error(`stepNameHeadersStream: ${err}`));
        stepLoggerWritableStream.on('error', err => logger.error(`stepLoggerWritableStream: ${err}`));

        stdout.pipe(logSizeLimitStream, {end: false}).pipe(stepNameHeadersStream, {end: false}).pipe(stepLoggerWritableStream, {end: false});

        if (!stderr) {
            return;
        }

        stdout.pipe(logSizeLimitStream, {end: false})
            .pipe(this._errorTransformerStream(), {end: false}).pipe(stepNameHeadersStream, {end: false}).pipe(stepLoggerWritableStream, {end: false});
