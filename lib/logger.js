    ...
    start() {
        ...
        logger.info(`Logging container created for logger id: ${this.loggerId}`);
          ...
          this.storageCsv.split(',').forEach( name =>{
            switch( name ) {
              case 'mongo':
                let getMongoStorage = MongoStorage.init(this.loggerId).catch( err=>{
                  this._error( new CFError({ cause: err, message: 'Cannot init MongoStorage'}) );
                });
                this.getStorages.push( getMongoStorage );
                break;
              case 'firebase': 
                let getFirebaseStorage = FirebaseStorage.init().catch( err=>{
                  this._error( new CFError( {cause: err, message: 'Cannot init FirebaseStorage'} ));
                });
                this.getStorages.push( getFirebaseStorage );
                break;
            }	
          });
