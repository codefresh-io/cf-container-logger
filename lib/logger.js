...
const { TaskLogger }          = require('@codefresh-io/cf-common');
...
class Logger {
    ...
    _handleContainer(container) {
        ...
        if (!stepName) {
            logger.error(`Container: ${containerId} does contain a stepName label`);
            return;
        }
        ...
        const stepLogger = this.taskLogger.create(stepName);
