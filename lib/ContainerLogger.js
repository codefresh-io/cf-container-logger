...
class ContainerLogger extends EventEmitter {
    ...
    _stepLogSizeExceeded() {
        return this.logSizeLimit && this.logSize > this.logSizeLimit;
    }

    _logMessageToFirebase(message, isError) {
        if ((this._stepLogSizeExceeded() || this.isWorkflowLogSizeExceeded()) && !isError) {
            if (!this.logExceededLimitsNotified) {
                this.logExceededLimitsNotified = true;
                message = `\x1B[01;93mLog size exceeded for ${this._stepLogSizeExceeded() ? 'this step' : 'the workflow'}. New logs will not be stored.\x1B[0m\r\n`;
            } else {
                return;
            }
        }
