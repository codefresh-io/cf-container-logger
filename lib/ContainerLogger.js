    ...
    start() {
        return Q.ninvoke(this.containerInterface, 'inspect')
            .then((inspectedContainer) => {
                console.log('in start() of ContainerLogger - TAKE 4');
                ...
                if (this.loggerStrategy === LoggerStrategy.ATTACH) {
                    ...
                    console.log('About to go into _getLogsStrategyStream');
    ...
    _getLogsStrategyStream() {
        ...
        return Q.all([
            Q.ninvoke(this.containerInterface, 'logs', {
                follow: 1, // TODO: eas changed from '1' in all fields (and fails if so)
                stdout: 1,
                stderr: 1
            })
        ]);
