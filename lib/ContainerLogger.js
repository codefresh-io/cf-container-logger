...
class ContainerLogger {
	...
	/*
		@param {Object[]} storages - array of logger classes
	*/
    constructor(containerId, containerInterface, firebaseLogger, firebaseLastUpdate, loggerStrategy, getStorages) {
    ...
    start() {
        console.log(`Container logger, start()`);        
    ...
    _handleTtyStream(stream, isError) {
        stream.on('data', (chunk) => {
            ...
            const message = buf.toString('utf8');
						...
						if ( this.getStorages ) {
							this.getStorages.forEach(getStorage=>{
								getStorage.then(inst=>{ 
                  inst.logMessage( new Buffer(payload).toString('utf8') );
                });
							});
						}					
            else {
              this._logMessageToFirebase(message, isError); // should be removed when impplmented in storages
            }
        ...
        stream.on('readable', () => {
            ...
            while (header !== null) {
                ...
                if (payload === null) {
                  ...
                  this._logMessageToFirebase(new Buffer(payload).toString('utf8'), isError); // should be removed when implemented in storages
                }
