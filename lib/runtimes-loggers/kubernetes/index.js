...
class KubernetesLogger extends Base {
    ...
    static async build({
        ...
        const client = new Client({ backend, version: 1.14 });
    ...
    async _handlePod(pod) {
        ...
        const readyContainers = _.reduce(unhandledContainers, (acc, stepName, containerName) => {
            ...
            if (!containerStatus.started) {
                logger.info(`Not handling container: ${containerName} pod: ${pod.metadata.name} because this container was not yet started`);
                delete acc[containerName];
            }
            return acc;
        }, unhandledContainers);
