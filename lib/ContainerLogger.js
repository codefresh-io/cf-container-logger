...
class ContainerLogger {
    ...
    constructor(containerId, containerInterface, loggerStrategy, logId, logUrl, stepId) {
    ...
    _handleTtyStream(stream, isError) {
        stream.on('data', (chunk) => {
            ...
            this._logMessageToStreamer(message, isError);
        });
    ...
    _handleNonTtyStream(stream, isError) {
        stream.on('readable', () => {
            ...
            while (header !== null) {
                ...
                this._logMessageToStreamer(new Buffer(payload).toString('utf8'), isError);
    ...
    _logMessageToStreamer(message, isError) {
        if (isError) {
            message = `\x1B[31m${message}\x1B[0m`;
        }

        // Write to the logs-streamer service
        ...
        var streamerBasePath = process.env.STREAMER_PATH | "";
        var options = {
          host: process.env.STREAMER_HOST || 'localhost',
          port: process.env.STREAMER_PORT || '8081',
          path: `${streamerBasePath}/logsdata/${jobId}/steps/${stepId}/logs/`,
          method: 'POST',
          json: true,
          headers: {
        ...
        var req = http.request(options, function(res) {
          // TODO: Do something in case of a failure
        });
