...
class KubernetesLogger extends Base {
    ...
    static async build({
        ...
        logger.info('loading api spec...');
        await client.loadSpec();
        logger.info(`kubernetes api spec loaded`);

        return new KubernetesLogger({ kubeClient: client, loggerId, taskLogger, state, logSizeLimit, namespace });
    }
    ...
    async _handlePod(pod) {
        ...
        // handle only containers that are in ready state
        const readyContainers = _.reduce(unhandledContainers, (acc, stepName, containerName) => {
            const containerStatus = containerStatuses[containerName];
            if (!containerStatus || Object.keys(containerStatus.state).includes('waiting')) {
                logger.info(`Not handling container: ${containerName} pod: ${pod.metadata.name} because this container is not ready yet`);
                delete acc[containerName];
            }
            return acc;
        }, unhandledContainers);
        ...
        return Promise.all(_.map(readyContainers, async (stepName, containerName) => {
            ...
            try {
                ...
                const logStream = await this.kubeClient.api.v1
                    .namespaces(pod.metadata.namespace)
                    .pods(pod.metadata.name)
                    .log
                    .getByteStream({ qs: { container: containerName, follow: true } });
    ...
    async listen() {
        let stream;
        if (this.namespace) {
            stream = await this.kubeClient.api.v1.watch.namespaces(this.namespace).pods.getObjectStream({ qs: { labelSelector: this.labelSelector } });
            logger.info(`starting to watch for pods with label-selector: ${this.labelSelector} in namespace: ${this.namespace}`);
        } else {
            stream = await this.kubeClient.api.v1.watch.pods.getObjectStream({ qs: { labelSelector: this.labelSelector } });
    ...
    async findExisting() {
        let res;
        if (this.namespace) {
            logger.info(`searching for existing pods with labelSelector: ${this.labelSelector} in namespace: ${this.namespace}`);
            res = await this.kubeClient.api.v1.namespaces(this.namespace).pods.get({ qs: { labelSelector: this.labelSelector } });
        } else {
            logger.info(`searching for existing pods with labelSelector: ${this.labelSelector}`);
            res = await this.kubeClient.api.v1.pods.get({ qs: { labelSelector: this.labelSelector } });
        }
        logger.info(`found [${_.get(res, 'body.items.length', 0)}] existing pods with matching labels`);
