...
class ContainerLogger {
    ...
    constructor(containerId, containerInterface, firebaseLogger, firebaseLastUpdate, firebaseMetricsLogs, loggerStrategy) {
    ...
    start() {
        return Q.resolve()
            .then(() => {
                ...
                this.firebaseMetricsLogs.child('total').once('value', (snapshot) => {
                    this.logSize = snapshot.val() || 0;
                    deferred.resolve();
                }, (err) => {
                    deferred.reject(err);
                });
