...
class ContainerLogger {
    ...
    constructor(containerId, containerInterface, firebaseLogger, firebaseLastUpdate, loggerStrategy, onDockerNodeTerminationHandler) {
        ...
        logger.warn("ContainerLogger created with onDockerNodeTerminationHandler:", onDockerNodeTerminationHandler);
    }
            ...
            .then(([stdout, stderr]) => {
                ...
                stdout.on('end', () => {
                    ...
                    if (this.onDockerNodeTerminationHandler &&
                        ...
                        logger.warn('Triggering Docker node termination handler (from stdout handler).');
                ...
                stderr.on('end', () => {
                    ...
                    if (this.onDockerNodeTerminationHandler &&
                        !this.dockerNodeTerminationHandlerFired) {
                        logger.warn('Triggering Docker node termination handler (from stderr handler).');
                        this.dockerNodeTerminationHandlerFired = true;
                        this.onDockerNodeTerminationHandler();
                    }
                });
