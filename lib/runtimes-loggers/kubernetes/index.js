...
class KubernetesLogger extends Base {
    ...
    constructor({
        ...
        this.kubeClient = {
            corev1: kubeconfig.makeApiClient(kc.CoreV1Api),
            watch: new kc.Watch(kubeconfig),
            attach: new kc.Attach(kubeconfig),
        };
    ...
    static async build({
        ...
        const kubeconfig = new kc.KubeConfig({});
        ...
        return new KubernetesLogger({ kubeconfig, loggerId, taskLogger, state, logSizeLimit, namespace });
    }
    ...
    async _handlePod(pod) {
        ...
        return Promise.all(_.map(readyContainers, async (stepName, containerName) => {
            ...
            try {
                ...
                const attachInterface = this.kubeClient.attach.attach.bind(
                    this.kubeClient.attach, // this
    ...
    async listen() {
        if (this.namespace) {
            await this.kubeClient.watch.watch(`/api/v1/namespaces/${this.namespace}/pods`, {
                labelSelector: this.labelSelector,
            }, (type, apiObj) => this._handlePod(apiObj));
            logger.info(`starting to watch for pods with label-selector: ${this.labelSelector} in namespace: ${this.namespace}`);
        } else {
            await this.kubeClient.watch.watch(`/api/v1/pods`, {
                labelSelector: this.labelSelector,
            }, (type, apiObj) => this._handlePod(apiObj));
    ...
    async findExisting() {
        ...
        if (this.namespace) {
            ...
            res = await this.kubeClient.corev1.listNamespacedPod(
                this.namespace,    // namespace
                undefined,         // pretty
            ...
            res = await this.kubeClient.corev1.listPodForAllNamespaces(
                undefined,         // allowWatchBookmarks
                undefined,         // _continue
                undefined,         // fieldSelector
                this.labelSelector // labelSelector
