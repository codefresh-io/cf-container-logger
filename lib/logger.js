...
class Logger {
    ...
    _handleContainer(container) {
        ...
        if (!receivedFirebaseLogsUrl) {
            ...
            // TODO: The runtime should probably add a label for the stepId so we won't need to extract it ourselves.
            var logsUrlPath = url.parse(receivedFirebaseLogsUrl).pathname;
            stepId = logsUrlPath.split('/').reverse()[1];
        }
        ...
        const containerLogger    = new ContainerLogger(containerId, containerInterface, loggerStrategy, receivedLoggerId, receivedFirebaseLogsUrl, stepId);
