...
const initialState = { pid: process.pid, status: 'init', lastLogsDate: Date.now() , failedHealthChecks: [] , restartCounter: 0, containers: {} };
class Logger {
        ...
        TaskLogger(this.taskLoggerConfig.task, this.taskLoggerConfig.opts)
            .then((taskLogger) => {
                ...
                taskLogger.onHealthCheckReported((status) => {
                    ...
                    this.state.logsStatus = this.taskLogger.getLogsStatus();
                    logger.info(`logs status is: ${JSON.stringify(this.state.logsStatus)}`);
    ...
    async _handleContainer(container) { // jshint ignore:line
        ...
        containerLogger.on('end', this._handleContainerStreamEnd.bind(this));
    ...
    _handleContainerStreamEnd() {
        ...
        if (this.containerLoggers.length === this.finishedContainers) {
            this.allStreamsClosed.resolve();
        }
    }
    ...
    _handleBuildFinished() {
        this.buildFinishedPromise
            .then(() => {
                ...
                logger.info(`logs status is: ${JSON.stringify(this.taskLogger.getLogsStatus())}`);
                return this.allStreamsClosed.promise;
            })
            .then(() => {
                logger.info('=== all streams have been closed ===');
                this.taskLogger.awaitLogsFlushed();
            })
            .then(() => {
