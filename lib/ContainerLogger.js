...
class ContainerLogger {
    ...
    constructor(containerId, containerInterface, loggerStrategy) {
    ...
    _handleTtyStream(stream, isError) {
        stream.on('data', (chunk) => {
            ...
            this._logMessageToRabbitMQ(message, isError);
        });
    ...
    _handleNonTtyStream(stream, isError) {
        stream.on('readable', () => {
            ...
            while (header !== null) {
                ...
                this._logMessageToRabbitMQ(new Buffer(payload).toString('utf8'), isError);
    ...
    _logMessageToRabbitMQ(message, isError) {
        if (isError) {
