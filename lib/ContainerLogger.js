...
class ContainerLogger extends EventEmitter {
    ...
    start() {
        return Q.ninvoke(this.containerInterface, 'inspect')
            ...
            .then(([stdout, stderr]) => {
                logger.info(`Attached stream to container: ${this.containerId}, job id: ${this.stepLogger.jobId}`);
                ...
                if (this.tty) {
                    ...
                    if (this.stepLogger.getConfiguration().logsRateLimitConfig) {
    ...
    _streamTty(stdout, stderr) {
        const logsWritableStream = this.stepLogger.writeStream();
        pipeline(stdout, this._logSizeLimitStream(), logsWritableStream, (err) => {
            if (err) {
        ...
        pipeline(stderr, this._logSizeLimitStream(), this._errorTransformerStream(), logsWritableStream, (err) => {
            if (err) {
