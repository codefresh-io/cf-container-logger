...
class Logger {
    ...
    start() {
        ...
        TaskLogger(this.taskLoggerConfig.task, { ...this.taskLoggerConfig.opts, updateLogsRate: true })
            .then((taskLogger) => {
                ...
                console.log(`BEFORE _listenForNewContainers()`);
                this._listenForNewContainers();
                console.log(`AFTER _listenForNewContainers()`);
    ...
    _readState() {
        ...
        if (fs.existsSync(filePath)) {
            console.log('INSIDE _readState() - file exists');
            ...
            console.log('INSIDE _readState() - file DOESNT exist');
    ...
    _writeNewState(disableLog = false) {
        console.log('INSIDE _writeNewState');
        const filePath = `${__dirname}/state.json`;
    ...
    async _handleContainer(container) { // jshint ignore:line
        console.log('INSIDE _handleContainer for container: ', JSON.stringify(container));
        ...
        console.log('logger Strategy in _handleContainer is:', loggerStrategy);
        ...
        this.state.containers[containerId] = { status: ContainerHandlingStatus.INITIALIZING }; // TODO: WHY? MAYBE THIS CAUSES IT TO GET STUCK ON INITIALIZING.
        ...
        console.log('here is containerInterface:', JSON.stringify(containerInterface));
        ...
        containerLogger.start()
            .done(() => {
                console.log('in done() of containerLogger.start - ABOUT TO CHANGE STATE TO LISTENING!');
                this.state.containers[containerId] = { status: ContainerHandlingStatus.LISTENING };
                this._writeNewState();
            }, (err) => {
                console.log('in err of containerLogger.start');
                const error = new CFError({
                    cause: err,
                    message: `Failed to start logging for container:${containerId}`, // TODO: doesnt appear in the logs...
                    containerId
                });
                logger.error(error.toString());
            });
    ...
    _updateMissingLogs() {
        ...
        console.log('SHOW THE CURRENT writeCalls: ', writeCalls);
        console.log('SHOW THE CURRENT resolvedCalls: ', resolvedCalls);
        console.log('SHOW THE CURRENT rejectedCalls: ', rejectedCalls);

        _.set(this, 'state.logsStatus.missingLogs', writeCalls - resolvedCalls - rejectedCalls);
    }

    _updateTotalLogSize() {
        console.log('INSIDE _updateTotalLogSize! meaning: an message.logged event WAS NOTICED!');
    ...
    _listenForNewContainers() {
        ...
        emitter.start(); // TODO: what is this start?
        emitter.on('create', this._handleContainer.bind(this));
        emitter.on('start', this._handleContainer.bind(this)); // TODO: my suspicion: a 'start' event never occurs. container stays initiailizing for some reason.
        // TODO: add a listener to 'data' event, no?
